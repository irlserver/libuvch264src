...
void cb(uvc_frame_t *frame, void *ptr) {
  ...
  static const char *H264_FILE = "DJI-output.h264";
  static const char *SPSPPS_FILE = "spspps.data";
char spspps[1024];
FILE* fp0 = fopen(SPSPPS_FILE, "rb");
int read_bytes = fread(spspps, 1, 1024, fp0);
fclose(fp0);
  ...
  case UVC_FRAME_FORMAT_H264:
    /* use `ffplay H264_FILE` to play */
     FILE* fp = fopen(H264_FILE, "a");
	if ((((char*)frame->data)[4] & 0x1F) == 5)
		fwrite(spspps, 1, read_bytes, fp);
...
printf(" *** %d %u\n", frame->frame_format, frame->sequence);
...
int main(int argc, char **argv) {
  ...
  if (res < 0) {
    ...
    if (res < 0) {
      ...
      format_desc = format_desc->next; // To use 2nd format(H.264). FIXME
